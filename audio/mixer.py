...
def mix_narration_with_bgm(
    ...
    Raises RuntimeError if narration_path is missing or FFmpeg fails.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"FFmpeg failed mixing {narration_path.name} → {output_path.name}: {stderr}"
        ) from exc

    if not output_path.exists() or output_path.stat().st_size == 0:
        raise RuntimeError(
            f"FFmpeg produced no output for {narration_path.name} → {output_path}"
        )
...
def find_bgm(assets_dir: Optional[str] = None) -> Optional[Path]:
    """Return first BGM file found in assets/music/ (sorted for determinism)."""
